...
const expected = [
  ...
  "data/dev-quick.json", "data/fa-junior.json", "data/fpa-junior.json", "data/software-junior.json", "data/tourism-junior.json"
];
...
assert.match(deploy, /\$publicFiles\.Count -ne 24[\s\S]*Count -ne 24/);
assert.match(deploy, /exactly 24 unique files/);
...
console.log("Yandex public-site deployment checks passed: exact 24-file boundary, verified upload and Yandex-only candidate CORS.");
