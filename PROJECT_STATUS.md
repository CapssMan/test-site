...
- Создан отдельный публичный контакт проекта `skillcheck.project@yandex.ru` и внесён в privacy/consent; 24.07.2026 владелец подтвердил проверку входящего и исходящего письма. ФИО/наименование и адрес оператора остаются legal-блокерами.
...
- По актуальным официальным страницам подготовлен закрытый discovery-longlist из 12 потенциальных reviewers: по три FA/FPA, CA, ACC и BI, с evidence, безопасными каналами и первой волной из четырёх обращений. Сообщения никому не отправлялись; наличие в longlist не считается согласием, проверкой компетенции или SME sign-off.
- Для первой волны вне Git подготовлены четыре персонализированных email-черновика: FA/FPA, CA, ACC и BI. Они не содержат вложений или ключей и имеют статус `DRAFT — NOT SENT`; отправка от имени владельца требует отдельного решения.
