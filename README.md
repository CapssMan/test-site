...
Этап 18 завершён с решением **LIMITED GO для контролируемого пилота**. Candidate Build `2026.07.31.1` опубликован в Yandex Object Storage и использует `assessment-v6`; admin работает через `admin-v3`, рейтинг — через `read-v4`/`write-v6`. Ровно 13 публичных файлов сверены по SHA-256, основной Yandex-origin проверен на реальных ответах, а GitHub-origin запрещён API, YDB пуста, pilot gates закрыты. Локальный CI выполняет 40 test suite и 5 infrastructure validators. GitHub Pages сохранён как статический rollback с переводом интерактивных страниц на Yandex; предыдущие function tags сохранены.

LEGAL_PILOT_APPROVED и ATTEMPT_ISSUANCE_ENABLED открыты отдельными действиями после публикации реквизитов оператора, фиксации уведомления Роскомнадзора, утверждения УЗ-4/угроз типа 3 и owner sign-off. Независимый человеческий SME review рекомендуется до открытого self-service запуска и масштабирования, но не является блокером малой контролируемой волны после внутренней проверки банков v5.
...
- отдельное согласие на обработку персональных данных версии `skillcheck-pd-consent-2026-07-31-v5`, связанное с attempt token/session и серверным временем;
