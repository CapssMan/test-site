...
В production работают 11 профессиональных направлений, 480 закрытых вопросов и account-first поток через Яндекс ID. Активные runtime: `assessment-v15`, `account-v6`, `admin-v13`, `employer-v4`, `read-v7`, `write-v9`. Ровно 31 публичный файл опубликован на основном Yandex-origin. `demo.html` использует только вымышленные данные, не обращается к API/хранилищу и не влияет на рейтинг. Локально подготовлен следующий безопасный UI-выпуск: живой предпросмотр карточки кандидата без открытия публикации профилей.
