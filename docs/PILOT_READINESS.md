...
## Актуализация 08.08.2026 — NO-GO до первичного уведомления
...
Независимый человеческий SME review остаётся рекомендацией перед масштабированием. 8 августа 2026 года выяснилось, что первичное уведомление Роскомнадзору не было отправлено. Поэтому `legal_pilot_approved`, `attempt_issuance_enabled`, `account_registration_enabled`, `profile_publication_enabled` и `employer_contact_enabled` принудительно выключены; автоматизация сроков хранения остаётся включённой: `retention_automation_enabled=true`.
Обновлено: 31 июля 2026 года, после ограничения production-origin российским Yandex-сайтом.

## Решение

**NO-GO до подтверждения первичной подачи.** Новые приглашения, попытки и аккаунты не выдаются. После отправки первичного уведомления требуется повторная проверка публичных документов и отдельное решение об открытии controlled pilot на 10–30 завершённых прохождений; публикация профилей и employer-контакты этим решением автоматически не открываются.
