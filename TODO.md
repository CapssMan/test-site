...
Техническая часть этапа 12 опубликована в deployment `@54`; legal approval, выдача приглашений и автоматический retention выключены. Следующий плановый этап — 13, резервные копии и восстановление; рекомендуемый режим — `высокий`. Отдельный pilot blocker — содержательная ротация банков с SME review; для неё нужен режим `ультра` и явное подтверждение пользователя. Спорные вопросы и методические риски зафиксированы в `docs/QUESTION_BANK_AUDIT.md`.
