...
Текущий спринт добавил MVP-механику кандидатского трекинга и минимизацию данных:

- телефон больше не собирается в стартовой форме и новых результатах;
- перед стартом требуется согласие на обработку персональных данных;
- перед стартом требуется подтверждение 18+;
- передача результата работодателям допускается только при отдельном согласии кандидата;
- добавлена страница `privacy.html`;
- повторная попытка одного и того же теста блокируется на 21 день по `testId` + email или browser fingerprint;
- в `Results` добавлены отдельные колонки `ID теста` и `Fingerprint`, чтобы блокировка не зависела от человекочитаемого названия теста;
- в браузере дополнительно сохраняется localStorage-lock `skillcheck_attempt_<testId>`;
...
Результат тестирования считается предварительной оценкой отдельных навыков. Он не должен использоваться как самостоятельное решение о найме, отказе в найме или профессиональной пригодности.
