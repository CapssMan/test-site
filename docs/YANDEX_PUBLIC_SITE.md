...
Статус на 8 августа 2026 года: российский статический контур опубликован и проверен, контролируемый пилот открыт. Отдельная V2 доступна только по прямой ссылке и закрыта от поисковой индексации. `SkillCheck` — временное рабочее имя.
...
В public bucket загружаются только 16 явно перечисленных файлов: восемь HTML-страниц, PNG-карточка, SVG-исходник и шесть display-only JSON-банков. Скрипт не использует рекурсивную загрузку, прекращает работу при постороннем объекте и после загрузки требует точного совпадения всего списка объектов.
...
Новый сервис не подключался: использованы уже созданные bucket, API Gateway и Cloud Function. В публичном бакете 16 объектов общим объёмом менее 1 МБ. CDN, Lockbox, VM и платный домен не подключены. При текущем тестовом трафике объём значительно ниже ежемесячных бесплатных квот, но бюджетное уведомление остаётся обязательным и при росте посещаемости стоимость нужно проверять отдельно.

## Повторная публикация

Из корня репозитория запускается `scripts/deploy-yandex-public-site.ps1`. Скрипт сначала проверяет allowlist и локальные контракты, обновляет gateway из `cloud/api-gateway.yaml`, загружает ровно 16 объектов, проверяет bucket и живые контрольные суммы, затем подтверждает Yandex-only browser CORS, отказ GitHub-origin и нейтральный отказ по недействительному приглашению.
