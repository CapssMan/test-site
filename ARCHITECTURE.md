...
Обновлено: 26 июля 2026 года, этап 18 (локальная основа рейтинга).
...
## Целевая российская схема этапа 18

```text
Браузер -> Yandex Object Storage (статический frontend)
...
Cloud-ресурсы пока не созданы. Локально готовы `cloud/ranking-core.js` и `cloud/ranking-handler.js`: они совместимы с async store-адаптером, следуют HTTP-формату Yandex Cloud Functions, отделяют публичную проекцию от исходного результата и завершаются fail closed. Текущий Google Apps Script/Яндекс Диск остаётся временным production-контуром при закрытых pilot gates до единого cutover.
...
Публичная страница выбирает одну из пяти профессий, не содержит mock-профилей и до подключения российского API показывает только безопасное состояние «рейтинг готовится». После подключения она принимает исключительно allowlisted поля read-контракта. До пяти добровольных подходящих участников место не показывается; percentile отсутствует.
