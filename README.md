...
Этап 17 и техническая ротация пяти production-банков v4 завершены с решением **NO-GO для реальных кандидатов**: опубликованы 240 новых вопросов/вариантов/ключей/ID, private/public split, fail-closed trust anchors и crash-safe atomic cutover. Dependency-free `npm test` и read-only GitHub Actions CI выполняют 21 regression suite и 5 infrastructure validators. Runtime выпуска: candidate `Build 2026.07.21.13`, admin `Build 2026.07.21.13`, backend `yandex-disk-mvp-2026-07-21-14`, deployment `@57`; API `attempt-v2`.

`LEGAL_PILOT_APPROVED` и `ATTEMPT_ISSUANCE_ENABLED` остаются `false`. Реальные кандидаты не допускаются до заполнения реквизитов оператора, внешнего legal/retention checklist, независимого человеческого SME sign-off банков v4, credential review, очистки smoke-данных и owner sign-off.
...
11. Не включать `LEGAL_PILOT_APPROVED` и `ATTEMPT_ISSUANCE_ENABLED` для реальных кандидатов до внешнего legal/retention checklist, SME sign-off v4, credential/data cleanup и owner sign-off.
