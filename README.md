...
Телефон в MVP больше не собирается. Повторная попытка блокируется по машинному `testId`, email и hash технического browser fingerprint. В Google Sheets для этого используются колонки `ID теста` и `Fingerprint`, а в браузере дополнительно сохраняется localStorage-lock на конкретный тест.
