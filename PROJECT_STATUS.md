...
- Candidate Build `2026.07.29.3` опубликован по адресу `https://assessment-b1gafbjd3dlh-web.website.yandexcloud.net/` и использует российский `/v1/assessment`. GitHub Pages сохранён как рабочий rollback; Apps Script — как legacy backend rollback, а не активный candidate write-path.
- Публичный bucket содержит ровно 13 разрешённых объектов общим объёмом 587 243 байт: семь HTML и шесть display-only JSON. Все живые объекты совпали с локальными по SHA-256; answer keys, private banks, отчёты и секреты не публикуются.
- После независимой проверки исправлен actual-response CORS: preflight был корректен, но старые функции отвечали GitHub origin. Активны `assessment-v5` (`d4enrn8gjdqtne9o7mnt`), `admin-v2` (`d4euharntpfhnqc8rotg`), `read-v3` (`d4ec9dhihcfph8kgrce8`) и `write-v5` (`d4ern801i6976qila1mo`); оба точных frontend origin проверены на реальных GET/POST, wildcard отсутствует.
...
- Рейтинг умеет отдельное opt-in согласие, псевдоним, version-aware публикацию, российскую proof-проверку результата через `assessment-v5`/YDB, хранение только hash management token, отзыв и TTL 365 дней. Публичная база пуста, mock-профилей нет.
