...
- Локально пройдены locked install и 39 проверок: 34 test-файла, 5 infrastructure validators, 240 production-вопросов без ошибок/предупреждений.
...
- Candidate backend yandex-disk-mvp-2026-07-27-23 и Build 2026.07.27.16 остаются опубликованы; Admin Build 2026.07.28.1 подключён к российскому `/v1/admin`. Известные технические коды исключаются из обычной аналитики.
...
- Следующее техническое действие: определить и перенести только необходимое operational state, затем выполнить закрытый end-to-end smoke candidate runtime и переключить write-path с явным rollback. SME/legal/owner sign-off всё ещё обязательны перед открытием gates.
