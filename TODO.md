...
Техническая часть этапа 11 опубликована в deployment `@52`; legal approval и выдача приглашений выключены. Следующий плановый этап — 12, безопасное удаление и срок хранения; рекомендуемый режим — `высокий`. Отдельный pilot blocker — содержательная ротация банков с SME review; для неё нужен режим `ультра` и явное подтверждение пользователя. Спорные вопросы и методические риски зафиксированы в `docs/QUESTION_BANK_AUDIT.md`.
