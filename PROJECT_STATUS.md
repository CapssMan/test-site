...
- Для будущей очистки составлен закрытый предварительный реестр: семь из девяти production-строк однозначно сопоставлены с документированными smoke-кодами, две строки пока не идентифицированы. Ничего не удалялось; агрегированный счётчик не является достаточным основанием для удаления.
...
- Число «3» означает только будущие нумерованные этапы (`18–20`), а не все оставшиеся задачи. Перед ними есть пять pre-pilot потоков: SME и legal сейчас в работе, реквизиты оператора и cleanup заблокированы решениями/evidence, owner sign-off ожидает их завершения.
