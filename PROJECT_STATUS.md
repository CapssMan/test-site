...
- Технически завершён и опубликован: этап 11 — versioned-согласие, factual privacy policy и независимый legal gate.
- Production rollout выполнен в существующий deployment `@52` без смены Web App URL; API/health и desktop/mobile QA пройдены.
- Следующий плановый этап: 12 — безопасное удаление и срок хранения, рекомендуемый режим `высокий`.
- Реальный пилот заблокирован до реквизитов оператора, внешнего legal checklist, удаления/retention и отдельно согласованной содержательной ротации банков; для ротации нужен режим `ультра`.
...
- `backendVersion: yandex-disk-mvp-2026-07-20-10`;
...
До реального пилота остаются внешние и пользовательские действия: реквизиты/статус/contact оператора, legal review уведомлений/локализации/трансграничности, утверждённый retention и техническое удаление этапа 12. Legal gate остаётся закрытым.

## Оценка до финала roadmap

- Осталось 9 плановых этапов (`12–20`): 58–105 часов, 410–840 тыс. токенов и 2–4 календарные недели пилота.
- Отдельная содержательная ротация банков: 50–100 часов, 400–800 тыс. токенов плюс SME review.
- До limited-pilot readiness (`12–17` + ротация): 88–165 часов / 700 тыс. – 1,40 млн токенов плюс внешняя legal/SME проверка.
- До конца roadmap вместе с ротацией: 108–205 часов / 810 тыс. – 1,64 млн токенов + 2–4 недели пилота.
...
- `LEGAL_PILOT_APPROVED` и `ATTEMPT_ISSUANCE_ENABLED` остаются `false`; реальных кандидатов приглашать нельзя до operator/legal/retention checklist, ротации и этапа 17.
...
Для завершения документов нужны полное наименование/ФИО оператора, его статус (физлицо/ИП/юрлицо), адрес и действующий email для обращений по персональным данным. До получения этих сведений разработка безопасно продолжается с этапа 12, а оба pilot gate остаются закрытыми.
