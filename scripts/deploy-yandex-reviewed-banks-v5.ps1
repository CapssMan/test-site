...
$privateDir = Join-Path $workspaceRoot "skillcheck-private-v5-ai-r1"
$baselinePrivateDir = Join-Path $workspaceRoot "skillcheck-private-v4-r3"
...
$releaseId = "review-v5-2026-07-31-ai-r1"
$expectedTests = [ordered]@{
  "fa-junior" = "FA Junior v5.0"
  "ca-junior" = "CA Junior v5.0"
  "fpa-junior" = "FP&A Junior v5.0"
  "acc-junior" = "ACC Junior v5.0"
  "bi-junior" = "BI Junior v5.0"
}
...
function Invoke-YdbJson([string]$query) {
  $queryFile = Join-Path ([IO.Path]::GetTempPath()) ("skillcheck-ydb-v5-" + [Guid]::NewGuid().ToString("N") + ".sql")
  try {
...
function Download-And-Verify([string]$uri, [string]$expectedSha, [long]$expectedBytes, [string]$label) {
  $target = Join-Path $env:TEMP ("skillcheck-bank-v5-verify-" + [Guid]::NewGuid().ToString("N") + ".json")
...
try {
  foreach ($requiredPath in @($privateDir, $baselinePrivateDir, $publicDir, $yc, $ydb, $node)) {
  ...
  & $node (Join-Path $PSScriptRoot "verify-reviewed-v5-artifacts.js") `
    --private-dir $privateDir --public-dir $publicDir --baseline-private-dir $baselinePrivateDir
  ...
  $pending = Get-Content -Raw -Encoding UTF8 (Join-Path $privateDir "private-bank-review-pending.v5.json") | ConvertFrom-Json
  if ([string]$manifest.releaseId -ne $releaseId -or [string]$pending.releaseId -ne $releaseId -or
      @($manifest.banks).Count -ne $expectedTests.Count) {
    throw "Unexpected reviewed v5 release manifest."
  }
  ...
  foreach ($testId in $expectedTests.Keys) {
    ...
    if ($null -eq $manifestEntry -or $null -eq $pendingEntry -or $files.Count -ne 1 -or
      ...
      throw "$testId reviewed-bank manifest mismatch."
    }
    $file = $files[0]
    ...
    Write-Host "$testId reviewed v5 object verified."
  }
  ...
  if ($stored.Count -ne 5 -or $pointers.Count -ne 5) { throw "YDB v5 cutover count mismatch." }
  foreach ($expected in $metadata) {
    ...
    if ($null -eq $actual -or $null -eq $pointer -or [string]$actual.bank_version -ne $expected.Version -or
        ...
        $actual.active -ne $true) { throw "$($expected.TestId) YDB v5 verification failed." }
  }
  Assert-GatesClosed (Invoke-YdbJson $gatesQuery)
  Write-Host "DONE: five reviewed v5 banks and both YDB version registries are verified; pilot gates remain closed."
} finally {
