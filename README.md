...
Техническая часть этапа 12 завершена и опубликована в существующем стабильном Web App deployment `@54` без смены URL. Production-версии: candidate `Build 2026.07.20.12`, admin `Build 2026.07.20.11`, backend `yandex-disk-mvp-2026-07-20-11`; API `attempt-v2`. Добавлено безопасное ручное удаление с preview, транзакционной копией, проверкой отсутствия и закрытым техническим журналом.

`LEGAL_PILOT_APPROVED` и `ATTEMPT_ISSUANCE_ENABLED` остаются `false`. Реальные кандидаты не допускаются до заполнения реквизитов оператора, внешнего legal checklist и отдельно согласованной содержательной ротации банков: удаление `correct` из текущих файлов не отменяет старые answer keys, уже попавшие в Git history, клоны и кэши.
...
11. Не включать `LEGAL_PILOT_APPROVED` и `ATTEMPT_ISSUANCE_ENABLED` для реальных кандидатов до внешнего legal checklist, удаления/retention и ротации банков.
...
disk:/skillcheck/admin/results.json       псевдонимизированные данные без открытых контактов
