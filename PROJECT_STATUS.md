...
- Этап 18A завершён: в изолированной папке `skillcheck-prod` развёрнуты YDB Serverless, read-only Cloud Function и API Gateway; `ranking.html` подключена к live API. Контракт принимает только добровольно опубликованные server-verified результаты текущей версии банка, исключает девять технических кодов и не возвращает имя, контакты, ответы, полный отчёт или код результата. База рейтинга пока пуста, mock-данных нет.
...
- Зафиксирована продуктовая North Star в `docs/PRODUCT_VISION.md`: self-service кандидаты, opt-in профили, version-aware рейтинг по профессиям и employer shortlist. Минимальный opt-in рейтинг перенесён в этап 18; сложный employer marketplace остаётся после короткого пилота.
...
- Разработка этапа 18 (российский runtime и рейтинг) может начинаться сейчас; реальные кандидаты и публичное включение остаются закрыты до короткого launch-checklist и SME sign-off.
...
- Осталось 4 проверяемых шага: write-path профиля, полный российский cutover, короткий пилот, публичный запуск.
- После работающей российской read-основы осталось примерно 36–63 часа, 140–290 тыс. токенов и 1–3 календарные недели пилота.
