...
Этап 10A технически реализован. Подготовлены candidate `Build 2026.07.20.11`, admin `Build 2026.07.20.9` и backend `yandex-disk-mvp-2026-07-20-9`: закрытые versioned-банки, одноразовые приглашения, подписанные attempt-токены и авторитетный серверный расчёт. Production rollout в существующий Web App ещё выполняется; deployment, production smoke и implementation commit: `pending`.

`ATTEMPT_ISSUANCE_ENABLED` остаётся `false`. Реальные кандидаты не допускаются до отдельно согласованной содержательной ротации банков: удаление `correct` из текущих файлов не отменяет старые answer keys, уже попавшие в Git history, клоны и кэши.
...
Важно: текущие public JSON больше не содержат правильных ответов, а frontend не считает итог. Однако прежние ключи уже опубликованы в истории Git и могут оставаться в клонах/кэшах. Поэтому технический 10A закрывает доверие к клиенту, но не делает текущий контент секретным заново. До реального пилота нужна отдельно подтверждённая содержательная ротация вопросов/вариантов/ключей с экспертной проверкой. Решение описано в [`docs/BACKEND_SCORING_DECISION.md`](docs/BACKEND_SCORING_DECISION.md).
...
11. Не включать `ATTEMPT_ISSUANCE_ENABLED` для реальных кандидатов до содержательной ротации банков и pilot checklist.
...
disk:/skillcheck/private/invites-v1.json  состояния одноразовых приглашений
disk:/skillcheck/private/attempt-sessions-v1.json состояния attempt и recovery
disk:/skillcheck/private/banks/<test>/<version>.json закрытые versioned-банки
```
...
Публичный контракт `attempt-v1` оставляет только POST `beginAttempt` и token-bound `saveResult`. Legacy/tokenless сохранение отклоняется как `client_upgrade_required`; публичный `checkAttempt` удалён. Сервер сверяет точный manifest, option IDs, bank version и одноразовое состояние сессии, затем сам рассчитывает балл. Частотные лимиты через Apps Script `CacheService` остаются best-effort защитой и не заменяют внешний API gateway/WAF.
...
Главный pilot blocker теперь не клиентский scoring, а историческая компрометация контента: старые answer keys остаются доступными вне текущей ветки. Перед включением выдачи приглашений нужны содержательная ротация, экспертная проверка и повторная миграция private/public банков. Для открытого публичного потока дополнительно нужны OTP/auth, CAPTCHA и/или внешний gateway.
...
Исторический production smoke этапа 10 подтвердил baseline `@49`. Production deployment и smoke именно для 10A пока `pending`; до их завершения нельзя считать подготовленные версии опубликованными.
...
Пути `YANDEX_DISK_INVITES_FILE`, `YANDEX_DISK_ATTEMPT_SESSIONS_FILE` и `YANDEX_DISK_PRIVATE_BANKS_FOLDER` необязательны: при их отсутствии используются закрытые пути из схемы выше. Секреты и digest anchors создаются owner-bootstrap, их значения нельзя коммитить или показывать в диагностике.
