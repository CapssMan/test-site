...
assert.match(testPage, /версии <b>21\.08\.2026-v6<\/b>/);
...
assert.match(testPage, /const FRONTEND_BUILD = "2026\.08\.21\.1"/, "candidate build must be current");
assert.match(testPage, />Build 2026\.08\.21\.1<\//, "visible candidate build must match submitted build");
assert.match(testPage, /const PRIVACY_CONSENT_VERSION = "skillcheck-pd-consent-2026-08-21-v6"/);
...
assert.match(testPage, /const RANKING_CONSENT_VERSION = "skillcheck-ranking-public-2026-07-31-v3"/);
...
assert.match(begin, /postBeginAttemptWithRetry\(payload, 0\)/);
const beginRetry = extractTopLevelFunction(testPage, "postBeginAttemptWithRetry");
assert.match(beginRetry, /postJsonOnce\(payload, BEGIN_NETWORK_TIMEOUT_MS\)/);
assert.match(beginRetry, /postBeginAttemptWithRetry\(payload, retryIndex \+ 1\)/, "begin retry must reuse the same payload and request id");
assert.match(beginRetry, /retryIndex >= MAX_AUTOMATIC_RETRIES/);
...
["fa-junior", "ca-junior", "fpa-junior", "acc-junior", "bi-junior", "tourism-junior", "software-junior", "product-project-junior", "sales-junior", "logistics-procurement-junior", "digital-marketing-junior"].forEach(testId => {
  assert.match(indexPage, new RegExp('href="test\\.html\\?test=' + testId + '"'), "home must retain " + testId);
});
assert.match(indexPage, /data-count="480"[^>]*>480<\//, "home must show the production bank size");
