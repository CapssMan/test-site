param([switch]$PublishTourism)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

if (-not $PublishTourism) { throw "Explicit -PublishTourism confirmation is required." }
...
$privateBank = Join-Path $workspaceRoot "skillcheck-private-tourism-r1\tourism-junior.json"
$publicBank = Join-Path $repoRoot "data\tourism-junior.json"
$bankVersion = "Tourism & Hospitality Operations Junior v1.1"
$bankObjectKey = "banks/tourism-v1/tourism-junior.json"
$runtimePlan = @(
...
$packagePath = Join-Path $env:TEMP ("skillcheck-tourism-runtime-" + [Guid]::NewGuid().ToString("N") + ".zip")
$verifyPath = Join-Path $env:TEMP ("skillcheck-tourism-bank-" + [Guid]::NewGuid().ToString("N") + ".json")
$publicVerifyPath = Join-Path $env:TEMP ("skillcheck-tourism-public-" + [Guid]::NewGuid().ToString("N") + ".json")
$packageUri = ""
$stageUri = ""
...
try {
  foreach ($required in @($yc, $ydb, $node, $gatewaySpec, $privateBank, $publicBank)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "Required deployment input is missing: $required" }
  }
  foreach ($validator in @("validate-tests.js", "audit-tourism-bank.js", "test-public-bank-secrecy.js", "check-static-links.js", "check-js-syntax.js")) {
    $arguments = @((Join-Path $PSScriptRoot $validator))
    if ($validator -eq "audit-tourism-bank.js") { $arguments += $privateBank }
    & $node @arguments
    if ($LASTEXITCODE -ne 0) { throw "Tourism validator failed: $validator" }
  }
  ...
  foreach ($key in @("legal_pilot_approved", "attempt_issuance_enabled", "account_self_service_enabled", "account_required_for_attempts", "profile_publication_enabled", "employer_workspace_enabled", "employer_contact_enabled")) {
    if (-not $previousSettings.ContainsKey($key)) { throw "Required runtime gate is missing: $key" }
  }
  if ($previousSettings.legal_pilot_approved -ne "true" -or $previousSettings.profile_publication_enabled -ne "false" -or
      $previousSettings.employer_workspace_enabled -ne "false" -or $previousSettings.employer_contact_enabled -ne "false") {
  ...
  if ($active.Count -ne 1 -or [long]$active[0].row_count -ne 0) { throw "Active assessment sessions block tourism cutover." }
  ...
  $privateSha = (Get-FileHash -LiteralPath $privateBank -Algorithm SHA256).Hash.ToLowerInvariant()
  $privateBytes = (Get-Item -LiteralPath $privateBank).Length
  $stageKey = "bank-staging/tourism-v1/tourism-junior-$privateSha.json"
  $stageUri = "s3://$packageBucket/$stageKey"
  $finalUri = "s3://$packageBucket/$bankObjectKey"
  & $yc storage s3 cp $privateBank $stageUri --only-show-errors
  if ($LASTEXITCODE -ne 0) { throw "Tourism private-bank staging upload failed." }
  & $yc storage s3 cp $stageUri $finalUri --only-show-errors
  if ($LASTEXITCODE -ne 0) { throw "Tourism private-bank promotion failed." }
  ...
  if ($LASTEXITCODE -ne 0 -or (Get-Item -LiteralPath $verifyPath).Length -ne $privateBytes -or
    ...
    throw "Tourism private-bank remote checksum mismatch."
  }
  $publicDigest = [string](Get-Content -Raw -Encoding UTF8 $publicBank | ConvertFrom-Json).publicDigest
  if ($publicDigest -notmatch '^[a-f0-9]{64}$') { throw "Tourism public digest is invalid." }
  $null = Invoke-YdbJson ("UPSERT INTO assessment_banks (test_id, bank_version, object_key, private_digest, public_digest, active, updated_at) VALUES " +
    "(Utf8('tourism-junior'), Utf8('" + (Escape-Yql $bankVersion) + "'), Utf8('" + (Escape-Yql $bankObjectKey) + "'), Utf8('$privateSha'), Utf8('$publicDigest'), true, CurrentUtcTimestamp()); " +
    "UPSERT INTO active_bank_versions (test_id, bank_version, updated_at) VALUES " +
    "(Utf8('tourism-junior'), Utf8('" + (Escape-Yql $bankVersion) + "'), CurrentUtcTimestamp());")
  $stored = @(Invoke-YdbJson "SELECT test_id, bank_version, object_key, private_digest, public_digest, active FROM assessment_banks WHERE test_id = Utf8('tourism-junior') AND active = true;")
  if ($stored.Count -ne 1 -or [string]$stored[0].bank_version -ne $bankVersion -or [string]$stored[0].private_digest -ne $privateSha -or [string]$stored[0].public_digest -ne $publicDigest) {
    throw "Tourism YDB metadata verification failed."
  }
  ...
  $script:packageObject = "packages/tourism-v1-$($script:packageSha).zip"
  ...
  $created = @{}
  foreach ($item in $runtimePlan) {
    $created[[string]$item.Target] = Create-Version $sources[[string]$item.Source] ([string]$item.Target) "Tourism v1.1 additive runtime"
  }
  $null = @(& $yc serverless api-gateway update --id $gatewayId --spec $gatewaySpec --no-logging --format json) | ConvertFrom-Json
  if ($LASTEXITCODE -ne 0) { throw "Tourism API Gateway cutover failed." }
  $gatewayUpdated = $true
  foreach ($item in $runtimePlan) {
    if ([string](Get-Version ([string]$item.Target)).id -ne [string]$created[[string]$item.Target].id) { throw "Runtime tag verification failed: $($item.Target)" }
  }

  & (Join-Path $PSScriptRoot "deploy-yandex-public-site.ps1") -SkipGatewayUpdate
  if ($LASTEXITCODE -ne 0) { throw "Tourism public-site deployment failed." }
  ...
  $ranking = Invoke-RestMethod -Method GET -Uri ($apiBase + "/v1/ranking?testId=tourism-junior&limit=1") -Headers @{ Origin = $origin } -TimeoutSec 30
  if ($ranking.ok -ne $true -or [string]$ranking.testId -ne "tourism-junior") { throw "Live tourism ranking verification failed." }
  Invoke-WebRequest -Method GET -Uri ($origin + "/data/tourism-junior.json") -UseBasicParsing -OutFile $publicVerifyPath -TimeoutSec 30
  if (-not (Test-Path -LiteralPath $publicVerifyPath) -or
      (Get-FileHash -LiteralPath $publicBank -Algorithm SHA256).Hash.ToLowerInvariant() -ne
      (Get-FileHash -LiteralPath $publicVerifyPath -Algorithm SHA256).Hash.ToLowerInvariant()) {
    throw "Live tourism public-bank verification failed."
  }
  Write-Host "DONE: tourism-junior v1.1 is live; six production directions are available; employer and contact gates remain closed."
} catch {
