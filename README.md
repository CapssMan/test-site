...
Содержание имеет статус **PASS для небольшого controlled pilot**, но не является независимым человеческим SME-заключением, официальной сертификацией или единственным основанием кадрового решения. Candidate/admin Build `2026.07.31.2` и runtime `assessment-v7` / `admin-v4` / `read-v5` / `write-v7` опубликованы в Yandex Cloud. Реальные попытки пока закрыты только до решения владельца по Роскомнадзору и финального owner sign-off; последующая психометрическая калибровка требует 10–30 реальных прохождений.
# SkillCheck
