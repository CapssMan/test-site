...
- Создан отдельный публичный контакт проекта `skillcheck.project@yandex.ru` и внесён в privacy/consent; 24.07.2026 владелец подтвердил входящее/исходящее письмо и двухфакторную защиту аккаунта. ФИО, статус НПД и регион Москва опубликованы; legal-блокером остаётся полный публичный адрес оператора.
...
- Первая reviewer-волна имеет статус `SENT — RESPONSE PENDING`: ответы, согласие на участие, проверка компетенции и SME sign-off пока не получены. Закрытые книги и ключи никому не передавались; до одного follow-up выдерживается 4–7 дней.
- Для legal/retention параллельно подготовлен закрытый запрос на бесплатную консультацию через МБМ: короткий телефонный сценарий, полный текст обращения, восемь прикладных вопросов и журнал evidence. Запрос пока не отправлялся; наличие черновика не является legal approval.
- Девять известных production smoke-кодов зафиксированы. После неуспешных безопасных попыток удаления владелец решил сохранить фактически присутствующие строки и исключить их из аналитики; новые попытки массового удаления прекращены.
- С 26.07.2026 владелец запретил любые новые внешние сообщения, follow-up, ответы и формы от его имени до технического завершения продукта. Для возможного самостоятельного обращения владельца в свой ВУЗ вне Git подготовлен отдельный institutional review pack; он не отправлен и не считается согласием/SME sign-off.
- Добавлен docs/PRE_PILOT_INPUTS.md: единый безопасный шаблон реквизитов оператора, legal/retention решения, SME verdict, проверки системного исключения smoke-кодов и owner sign-off. Закрытая owner-копия с известными данными и явными пробелами создана вне Git; персональные данные не публикуются.
...
- Этап 18 заблокирован до реквизитов оператора, внешнего legal/retention checklist, SME sign-off и owner sign-off; исторический smoke-cleanup заменён проверяемым исключением известных кодов.
...
- Число «3» означает только будущие нумерованные этапы (18–20), а не все оставшиеся задачи. Перед ними есть четыре активных pre-pilot потока: SME, legal/retention, реквизиты оператора и owner sign-off; исторический cleanup закрыт системным исключением технических кодов.
...
- В служебных JSON могут оставаться известные DEV- и FA-smoke записи; админка .15 фильтрует весь набор девяти кодов, а не только dev-quick.
...
- В `privacy.html` и `consent.html` опубликованы ФИО, статус НПД, регион и project email; явно выделенной заглушкой остаётся полный публичный адрес оператора, а legal gate не открывается до полного checklist.
- Не приглашать реальных кандидатов до checklist этапа 17.

## Ручной шаг пользователя

Project email проверен владельцем, а ФИО, статус НПД и регион оператора опубликованы. Для завершения документов нужен утверждённый профильным специалистом полный публичный адрес оператора. Дополнительно нужны независимый SME sign-off и внешнее legal/retention решение. До этого оба pilot gate остаются закрытыми.
