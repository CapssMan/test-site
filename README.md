...
Телефон в MVP больше не собирается. Повторная попытка проверяется через hash `testId + email + browserFingerprint + ATTEMPT_HASH_SALT`. Сырые email и fingerprint не хранятся в анти-повторе.
