...
- Технически завершён и опубликован: этап 12 — безопасное ручное удаление с preview, транзакционной копией, проверкой и журналом.
- Production rollout выполнен в существующий deployment `@54` без смены Web App URL; API/health, полный локальный набор проверок и desktop/mobile QA пройдены.
- Следующий плановый этап: 13 — резервные копии и восстановление, рекомендуемый режим `высокий`.
- Реальный пилот заблокирован до реквизитов оператора, внешнего legal/retention checklist, этапов 13–17 и отдельно согласованной содержательной ротации банков; для ротации нужен режим `ультра`.
...
- `backendVersion: yandex-disk-mvp-2026-07-20-11`;
...
- Осталось 8 плановых этапов (`13–20`): 51–93 часа, 350–720 тыс. токенов и 2–4 календарные недели пилота.
- Отдельная содержательная ротация банков: 50–100 часов, 400–800 тыс. токенов плюс SME review.
- До limited-pilot readiness (`13–17` + ротация): 81–153 часа / 640 тыс. – 1,28 млн токенов плюс внешняя legal/SME проверка.
- До конца roadmap вместе с ротацией: 101–193 часа / 750 тыс. – 1,52 млн токенов + 2–4 недели пилота.
...
- Ручное удаление реализовано; нет регулярных rotating backup/restore и утверждённого автоматического retention — этап 13 и внешний legal checklist.
...
Для завершения документов нужны полное наименование/ФИО оператора, его статус (физлицо/ИП/юрлицо), адрес и действующий email для обращений по персональным данным. До получения этих сведений разработка безопасно продолжается с этапа 13, а оба pilot gate остаются закрытыми.
