...
С 10 августа 2026 года candidate-поток работает в account-first режиме: Build `2026.08.09.1`, `assessment-v13` и `account-v2`. Кандидат выбирает любой из пяти тестов, входит через Яндекс ID и получает одну новую попытку по каждому тесту раз в 21 день. Ровно 22 публичных файла сверены по SHA-256, основной Yandex-origin проходит API/CORS, а GitHub-origin не получает candidate API. Локальный CI выполняет 62 test suite и 5 infrastructure validators.
