...
const expected = [
  ...
  "data/dev-quick.json", "data/fa-junior.json", "data/fpa-junior.json", "data/tourism-junior.json"
];
...
assert.match(deploy, /\$publicFiles\.Count -ne 23[\s\S]*Count -ne 23/);
assert.match(deploy, /exactly 23 unique files/);
...
console.log("Yandex public-site deployment checks passed: exact 23-file boundary, verified upload and Yandex-only candidate CORS.");
