...
Повторная попытка блокируется по машинному `testId`, email и телефону. В Google Sheets для этого используется отдельная колонка `ID теста`, чтобы название теста можно было менять без поломки проверки.
