...
В production работают 11 профессиональных направлений, 480 закрытых вопросов и account-first поток через Яндекс ID. Активные runtime: `assessment-v15`, `account-v6`, `admin-v13`, `employer-v4`, `read-v7`, `write-v9`. Ровно 31 публичный файл опубликован на основном Yandex-origin. `demo.html` использует только вымышленные данные, не обращается к API/хранилищу и не влияет на рейтинг. В личном кабинете работает живой локальный предпросмотр карточки кандидата; блок доказательств опыта допускает только проверенные и разрешённые работодателю записи, не раскрывая приватные сведения или ссылку на доказательство. Публикация профилей и раздел регалий остаются закрытыми. UI проверки достижений подготовлен полностью: кандидат видит трёхшаговый процесс, пояснение статуса и причину отклонения; оператор должен отметить фактическую сверку приватной ссылки, а для отклонения указать обязательный комментарий. Закрытый employer-интерфейс дополнен корректными категориями всех 12 шаблонов ролей и живым брифом вакансии: роль, формат и регион переносятся из поиска, перед отправкой показываются краткое предложение и полнота 5 обязательных полей.
...
Работодатель сможет открыть общий каталог, найти любого добровольно видимого кандидата по фильтрам, добавить его в shortlist и отправить выбранной группе структурированное приглашение. Кандидат увидит его в личном кабинете и сможет ответить «Интересно», «Нужны подробности» или «Неинтересно»; статусы синхронизируются без раскрытия email и телефона.

Production-проверка подтвердила:

- `legal_pilot_approved=true`, `attempt_issuance_enabled=true` и весь account-first контур продолжают работать;
- `employer_invitation_v1_schema=true`;
- `profile_publication_enabled=false`, `employer_workspace_enabled=false`, `employer_invitation_enabled=false`, `employer_contact_enabled=false`;
- старые runtime-версии сохранены; удалены только устаревшие теги `admin-v9` и `assessment-v11` из-за лимита тегов;
- полный CI проходит 75/75 проверок.

Каталог, shortlist и invitation-flow готовы технически, но не доступны работодателям до отдельного допуска проверенной организации и явного открытия трёх необходимых gates. Контакты и чат остаются отдельным последующим решением. Полный контракт: `docs/EMPLOYER_INVITATIONS.md`.
