...
Техническая часть этапа 13 опубликована в deployment `@55`; legal approval, выдача приглашений и автоматический retention выключены. Следующий плановый этап — 14, защищённая наблюдаемость и диагностика; рекомендуемый режим — `высокий`. Отдельный pilot blocker — содержательная ротация банков с SME review; для неё нужен режим `ультра` и явное подтверждение пользователя. Спорные вопросы и методические риски зафиксированы в `docs/QUESTION_BANK_AUDIT.md`.
