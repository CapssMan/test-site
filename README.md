...
Этап 17 технически завершён с решением **NO-GO для реальных кандидатов**: пять тестов, desktop/mobile, public smoke, owner diagnostics, backup, monitoring и rollback проверены; обязательные внешние/содержательные блокеры собраны в `docs/PILOT_READINESS.md`. Dependency-free `npm test` и read-only GitHub Actions CI теперь выполняют 19 regression suites и 5 infrastructure validators. Production-версии не менялись: candidate `Build 2026.07.20.12`, admin `Build 2026.07.20.12`, backend `yandex-disk-mvp-2026-07-20-13`, deployment `@56`; API `attempt-v2`.
