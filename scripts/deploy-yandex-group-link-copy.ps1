...
$targetTag = "admin-v10"
$retiredTag = "admin-v7"
$packagePath = Join-Path $env:TEMP ("skillcheck-group-link-copy-" + [Guid]::NewGuid().ToString("N") + ".zip")
$packageUri = ""
...
try {
  foreach ($path in @($yc, $gatewaySpec, (Join-Path $repoRoot "cloud\admin-handler.js"))) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Required deployment input is missing." }
  }
  foreach ($validator in @("test-group-invites.js", "test-yandex-admin-runtime.js", "test-group-link-copy-deployment.js")) {
    & node (Join-Path $PSScriptRoot $validator)
    if ($LASTEXITCODE -ne 0) { throw "Group-link copy validator failed: $validator" }
  }
  ...
  if ($LASTEXITCODE -ne 0 -or $archiveEntries -notcontains "admin-core.js" -or $archiveEntries -notcontains "admin-handler.js" -or
  ...
  $packageObject = "packages/group-link-copy-$packageSha.zip"
  ...
  $createdRaw = & $yc serverless function version create --function-id $functionId --runtime ([string]$source.runtime) `
    ...
    --description "Protected group-link recovery successor of admin-v9" --environment $environment `
  ...
  Write-Host ("DONE: admin-v10 deployed and routed; admin-v9 remains rollback. Version id: " + [string]$created.id)
} finally {
